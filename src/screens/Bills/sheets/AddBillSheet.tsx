...
export function AddBillSheet({ open, onClose, onSave }: AddBillSheetProps) {
  ...
  const handleSave = () => {
    ...
    onSave({
      name:               name.trim(),
      vendor:             vendor.trim(),
      amount:             billType === 'variable' ? 0 : Number(amount),
      dueDate,
      paid:               false,
      autoPay,
    ...
    <BottomSheet open={open} onClose={onClose} snapPercent={95}>
      <div className="pt-2 pb-8">
        ...
        <div className="mb-4">
          ...
          <p style={{ fontSize: 11, color: '#8A7E6B', marginTop: 5 }}>
            {billType === 'fixed'
              ? 'Same amount every cycle (e.g. internet, subscriptions)'
              : 'Amount changes each cycle (e.g. electricity, water)'}
          </p>
        </div>
        ...
        <div className="mb-4">
          {imageUrl ? (
            ...
            <button onClick={() => fileRef.current?.click()} disabled={uploading} className="w-full h-16 rounded-xl flex items-center justify-center gap-2" style={{ background: '#F0E5D2', border: '2px dashed #E8DFCB', cursor: 'pointer' }}>
              ...
              <span style={{ fontSize: 12, color: '#8A7E6B', fontWeight: 600 }}>{uploading ? 'Uploading…' : 'Add bill photo'}</span>
            </button>
          )}
        ...
        <div className="bg-surface border border-hairline rounded-card overflow-hidden mb-4">
          ...
          {/* Only show fixed amount field for fixed bills */}
          {billType === 'fixed' && (
            <div className={rowCls}>
          ...
          <div className="rounded-[14px] p-4 mb-4" style={{ background: '#FFF8EC', border: '1.5px solid #DBA03A33' }}>
            ...
            <div className="flex items-center justify-between mb-3">
              ...
              <select
                ...
                style={{ background: '#F0E5D2', border: 'none', color: '#334266' }}
              >
                {ARRIVAL_DAYS.map((d) => (
