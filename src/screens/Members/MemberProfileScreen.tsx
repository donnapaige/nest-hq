  ...
  return (
    <div className="bg-bg h-full font-sans text-ink relative overflow-hidden flex flex-col">
      <div className="h-full overflow-y-auto" style={{ paddingBottom: 110 }}>
        ...
        <div className="flex px-5 pt-4 gap-2">
          {(['records', 'routines', 'notes'] as ProfileTab[]).map((t) => (
            <button
              ...
              style={{
                background: tab === t ? '#334266' : '#F0E5D2',
                color:      tab === t ? '#fff'    : '#8A7E6B',
        ...
        {tab === 'records' && (
          <div className="px-5 mt-4">
              ...
              {(['all', ...RECORD_TYPES] as (RecordType | 'all')[]).map((t) => {
                ...
                return (
                  <button
                    ...
                    style={{
                      background: filterType === t ? '#334266' : '#F0E5D2',
                      color:      filterType === t ? '#fff'    : '#8A7E6B',
                      border: 'none', cursor: 'pointer',
                    }}
                  >
            ...
            <button
              ...
              className="w-full mt-3 py-3 rounded-xl flex items-center justify-center gap-2 font-semibold text-sm"
              style={{ background: '#DCE0EB', color: '#334266', border: 'none', cursor: 'pointer' }}
            >
              <Icon name="plus" size={16} color="#334266" />
              Add record
            </button>
            ...
            {filteredRecords.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p style={{ fontSize: 13, color: '#8A7E6B', marginTop: 4 }}>Add milestones, health notes, allergies, or appointments.</p>
              </div>
            ) : (
                      ...
                      <div className="flex items-start justify-between gap-2">
                        <div className="flex items-start gap-2 flex-1">
                          ...
                          <div className="flex-1">
                            ...
                            {rec.recordDate && (
                              <p style={{ fontSize: 11, color: '#8A7E6B', marginTop: 4 }}>
                                📅 {new Date(rec.recordDate).toLocaleDateString('en-PH', { year: 'numeric', month: 'long', day: 'numeric' })}
                              </p>
                            )}
                          </div>
                        </div>
          ...
          <div className="px-5 mt-4">
            ...
            {routines.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p style={{ fontSize: 13, color: '#8A7E6B', marginTop: 4 }}>Add morning and evening routine steps.</p>
              </div>
            ) : (
        ...
        {/* ─── Notes tab (shortcut to appointment + note records) ─ */}
        {tab === 'notes' && (
          <div className="px-5 mt-4">
            <button
              onClick={() => { setRecType('note'); setShowAddRecord(true); }}
              className="w-full py-3 rounded-xl flex items-center justify-center gap-2 font-semibold text-sm mb-3"
              style={{ background: '#DCE0EB', color: '#334266', border: 'none', cursor: 'pointer' }}
            >
              <Icon name="plus" size={16} color="#334266" />
              Add note or appointment
            </button>
            {records.filter((r) => r.type === 'note' || r.type === 'appointment').length === 0 ? (
              ...
              <div className="flex flex-col gap-2.5">
                {records
                  .filter((r) => r.type === 'note' || r.type === 'appointment')
                  .map((rec) => {
                    const meta = TYPE_META[rec.type];
                    return (
                      <div key={rec.id} className="rounded-[14px] px-4 py-4" style={{ background: '#FBF8F1', border: '1px solid #E8DFCB' }}>
            ...
            <div className="px-5 py-5 space-y-4">
              ...
              <div>
                ...
                <input type="text" value={recTitle} onChange={(e) => setRecTitle(e.target.value)}
                  ...
                  style={{ borderColor: '#E8DFCB', background: '#fff' }}
                />
              </div>
              <div>
            ...
            <div className="px-5 py-5 space-y-4">
              ...
              <div>
                ...
                <input type="text" value={rotTime} onChange={(e) => setRotTime(e.target.value)}
                  placeholder="e.g. 7:00 AM, After school, Before bed"
                  className="w-full px-4 py-3 rounded-xl border text-sm outline-none"
                  style={{ borderColor: '#E8DFCB', background: '#fff' }}
                />
              </div>
